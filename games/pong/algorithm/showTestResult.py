...
data = pd.read_csv('output/results_algorithm_series3.csv')
...
for normalization in data['normalization'].unique():
    ...
    ax1.scatter(subset['architecture_encoded'], subset['log_lr'], subset['accuracy'], c=subset['accuracy'],
                cmap="inferno")
    ...
    # Heatmap
    heatmap_data = subset.pivot_table(index='architecture_encoded', columns='log_lr', values='accuracy',
                                      aggfunc=np.mean)
    ...
    ax3.scatter(subset['architecture_encoded'], subset['log_lr'], subset['classification_accuracy'],
                c=subset['classification_accuracy'], cmap="inferno")
